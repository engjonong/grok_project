...
def compute_layer_outputs(model, dataset, device, N=2000, batch_size=50):
...
with tqdm(total=optimization_steps) as pbar:
    for x, labels in islice(cycle(train_loader), optimization_steps):
        ...
        if steps % 1000 == 0 and steps > 0:
            try:
                layer_outputs, test_labels_for_save = compute_layer_outputs(mlp, test, device, N=len(test), batch_size=batch_size)
                #print(layer_outputs)
                if layer_outputs:
...
layer_outputs, test_labels = compute_layer_outputs(mlp, test, device, N=len(test), batch_size=batch_size)
if layer_outputs:
    out_dir = Path("layer_outputs")
    out_dir.mkdir(exist_ok=True)
    ...
    try:
        np.savez_compressed(
            data_path,
            log_steps=np.asarray(log_steps, dtype=np.float64),
